...
#define KALMAN_R_MEAS_DEFAULT                                                  \
  5.0f /* 降低：更信任实测坐标，快速目标响应更及时，原值11.0偏保守 */
 ...
 * 后续如果换引脚，只改这里即可。
 */
#define KEY_TOILET_PORT GPIOC
#define KEY_TOILET_PIN GPIO_PIN_0

#define KEY_EXIT_PORT GPIOC
#define KEY_EXIT_PIN GPIO_PIN_1

#define KEY_OFFICE_PORT GPIOE
#define KEY_OFFICE_PIN GPIO_PIN_13
...
/* 节奏模式定义：单位 ms */
/* 节奏模式定义：通过极短的脉冲（8~10ms）让蜂鸣器发出清脆的“滴答”声，感官音量更小
 */
...
#define GUIDE_BLOCK_AFTER_FOUND_MS                                             \
  (VOICE_TARGET_PROMPT_DELAY_MS + VOICE_STATUS_GUARD_MS)
...
/* 将搜索更新周期与主循环(8ms)严格对齐，消除"快慢停"错峰步级跳频造成的卡顿感。步长等比例缩放(1.2*8/25
 * ≈ 0.4) */
...
#define TARGET_CONFIRM_ENTER_FRAMES                                            \
  3U /* 降低：减少首次确认延迟约1帧，原值4U                    \
      */
...
uint8_t buzzer_pending_beeps = 0U;
...
SystemVoiceConfig g_system_voice_cfg = {
    .target_prompt_indices = {DFPLAYER_MODE_TOILET_MP3_INDEX,
...
const char *target_mode_name[TARGET_MODE_COUNT] = {
    "Toilet", "Exit", "Office", "Stairs", "Elevator", "Exting.", "None"};
...
void Kalman1D_Init(Kalman1D *kf, float init_pos, float init_vel, float q_pos,
                   float q_vel, float r);
...
void Update_TargetPresence_State(uint8_t frame_has_target,
                                 int32_t area_permille, uint32_t now_tick);
uint8_t Is_TargetPresence_Valid(void);

/* 用于做浮点数限幅，增强代码可读性和可移植性 */
float Limit_Float(float value, float min, float max);
uint8_t Parse_OpenMV_Message(const char *msg, int *x, int *y,
                             int *area_permille);
uint8_t Try_Dequeue_OpenMV_Line(char *dst, uint16_t dst_len);
uint8_t Should_Refresh_OLED(uint32_t now_tick, int32_t disp_x, int32_t disp_y,
                            int32_t disp_a, uint8_t force_refresh);
...
GuideForwardLevel_t Get_GuideForwardLevel_FromAreaByMode(uint8_t mode,
                                                         int32_t area_permille);
GuideForwardLevel_t
Get_GuideForwardLevel_FromRatio(float ratio_permille,
  ...
  while (1) {
      ...
      if (has_latest_line) {
        ...
        if (parse_count >= 2) {
          ...
          if (current_target_mode == TARGET_MODE_NONE) {
            ...
            if (frame_has_target && (area_permille >= 0)) {
              if (guide_target_area_permille < 0) {
                ...
                guide_filtered_area_permille =
                    (guide_filtered_area_permille *
                     (1.0f - GUIDE_AREA_FILTER_FACTOR)) +
                    ((float)area_permille * GUIDE_AREA_FILTER_FACTOR);
              }

              guide_target_area_permille =
                  (int32_t)(guide_filtered_area_permille + 0.5f);
            } else {
              guide_target_area_permille = -1;
              guide_filtered_area_permille = 0.0f;
            }
            ...
            Update_TargetPresence_State(frame_has_target,
                                        guide_target_area_permille, now_tick);
            ...
            if ((!prev_target_valid) && target_valid) {
              ...
              if (mode_switch_guard_active || mode_switch_pending_search) {
                ...
                SystemVoice_PostEvent(
                    &g_system_voice_state, VOICE_EVT_TARGET_LOST,
                    (TargetMode_t)current_target_mode, lost_tick);
                guide_enable_after_tick =
                    lost_tick + VOICE_LOST_PROMPT_DELAY_MS;
            ...
            if (frame_has_target && target_valid) {
              ...
              if (kalman_last_update_tick != 0U) {
                kalman_dt_s =
                    (float)(now_tick - kalman_last_update_tick) / 1000.0f;
                kalman_dt_s =
                    Limit_Float(kalman_dt_s, KALMAN_DT_MIN_S, KALMAN_DT_MAX_S);
              }
              ...
              in_final_tune_zone =
                  ((filtered_x >= X_CENTER_MIN) &&
                   (filtered_x <= X_CENTER_MAX) &&
                   (filtered_y >= Y_CENTER_MIN) && (filtered_y <= Y_CENTER_MAX))
                      ? 1U
                      : 0U;
              ...
              abs_lock_x_error =
                  (lock_x_error >= 0.0f) ? lock_x_error : (-lock_x_error);
              abs_lock_y_error =
                  (lock_y_error >= 0.0f) ? lock_y_error : (-lock_y_error);
              ...
              if (target_track_locked) {
                if ((abs_lock_x_error > TRACK_LOCK_EXIT_X_ERR_PIXELS) ||
                  ...
                  if (target_track_unlock_candidate_tick == 0U) {
                    ...
                    if (((now_tick - target_track_unlock_candidate_tick) >=
                         TRACK_LOCK_EXIT_STABLE_MS) ||
                        (target_track_unlock_candidate_frames >=
                         TRACK_LOCK_EXIT_CONFIRM_FRAMES)) {
                ...
                if (in_final_tune_zone &&
                  ...
                  } else if ((now_tick - target_track_lock_candidate_tick) >=
                             TRACK_LOCK_STABLE_MS) {
              ...
              if (target_track_locked) {
                ...
                raw_step_x = (x_error / IMAGE_CENTER_X) *
                             X_TRACK_STEP_GAIN_DEG * X_TRACK_DIRECTION;

                if ((filtered_x >= X_CENTER_MIN) &&
                    (filtered_x <= X_CENTER_MAX)) {
                  /* 中心区只减速，不冻结，提升反向穿越时的跟随性 */
                  raw_step_x *= X_CENTER_SLOW_FACTOR;
                  raw_step_x = Limit_Float(raw_step_x, -X_CENTER_STEP_MAX_DEG,
                                           X_CENTER_STEP_MAX_DEG);
                } else {
                  raw_step_x = Limit_Float(raw_step_x, -X_TRACK_STEP_MAX_DEG,
                                           X_TRACK_STEP_MAX_DEG);
                }

                /* 轻度平滑 step：smooth_step 往 raw_step
                 * 靠拢，避免台阶感又不过度变肉 */
                ...
                target_angle_x = Limit_Float(target_angle_x, SERVO_X_MIN_ANGLE,
                                             SERVO_X_MAX_ANGLE);

                {
                  float raw_step_y = (y_error / IMAGE_CENTER_Y) *
                                     Y_TRACK_STEP_GAIN_DEG * Y_TRACK_DIRECTION;

                  if ((filtered_y >= Y_CENTER_MIN) &&
                      (filtered_y <= Y_CENTER_MAX)) {
                    /* 中心区只减速，不冻结，与 X 轴策略统一 */
                    raw_step_y *= Y_CENTER_SLOW_FACTOR;
                    raw_step_y = Limit_Float(raw_step_y, -Y_CENTER_STEP_MAX_DEG,
                                             Y_CENTER_STEP_MAX_DEG);
                  } else {
                    raw_step_y = Limit_Float(raw_step_y, -Y_TRACK_STEP_MAX_DEG,
                                             Y_TRACK_STEP_MAX_DEG);
                  }

                  /* 轻度平滑 step：与 X 轴同机制，消除台阶感 */
                  smooth_step_y =
                      (smooth_step_y * (1.0f - Y_STEP_SMOOTH_ALPHA)) +
                      (raw_step_y * Y_STEP_SMOOTH_ALPHA);
                  step_y = smooth_step_y;

                  target_angle_y = current_angle_y + step_y;
                  target_angle_y = Limit_Float(
                      target_angle_y, SERVO_Y_MIN_ANGLE, SERVO_Y_MAX_ANGLE);
                }
              }

              if (Should_Refresh_OLED(
                      now_tick, (int32_t)filtered_x, (int32_t)filtered_y,
                      (int32_t)guide_target_area_permille, 0U)) {
    ...
    if (target_valid) {
      ...
      if (current_target_mode == TARGET_MODE_NONE) {
        ...
        if (Search_IsActive_FromModule()) {
          uint32_t now_tick = HAL_GetTick();
          if (SearchControl_Update(
                  &g_search_control_state, &g_search_control_cfg, now_tick,
                  target_valid, (TargetMode_t)current_target_mode,
...
uint8_t Try_Dequeue_OpenMV_Line(char *dst, uint16_t dst_len) {
  ...
  uart2_queue_read_idx =
      (uint8_t)((uart2_queue_read_idx + 1U) % UART2_RX_QUEUE_LEN);
  uart2_queue_count--;
  __enable_irq();
...
void Kalman1D_Predict(Kalman1D *kf, float dt_s) {
  ...
  kf->p00 = p00_old + (dt_s * (p10_old + p01_old)) + (dt_s * dt_s * p11_old) +
            kf->q_pos;
 ...
 */
void Reset_TargetCoordinateFilters(void) {
  Kalman1D_Init(&kf_x, IMAGE_CENTER_X, 0.0f, KALMAN_Q_POS_DEFAULT,
                KALMAN_Q_VEL_DEFAULT, KALMAN_R_MEAS_DEFAULT);
  Kalman1D_Init(&kf_y, IMAGE_CENTER_Y, 0.0f, KALMAN_Q_POS_DEFAULT,
                KALMAN_Q_VEL_DEFAULT, KALMAN_R_MEAS_DEFAULT);
...
void Update_TargetPresence_State(uint8_t frame_has_target,
  ...
  if ((area_permille >= 0) &&
      ((uint32_t)area_permille < TARGET_CONFIRM_MIN_AREA_PERMILLE)) {
    area_ok = 0U;
  }
  ...
  case TARGET_PRESENCE_CANDIDATE:
    if (frame_has_target) {
      ...
      if ((target_presence_hit_count >= TARGET_CONFIRM_ENTER_FRAMES) &&
          area_ok) {
  ...
  case TARGET_PRESENCE_HOLD:
    if (frame_has_target) {
      ...
      uint8_t timeout_reached =
          ((now_tick - target_presence_hold_start_tick) >= hold_timeout_ms)
              ? 1U
              : 0U;
 ...
 */
void Start_NormalSearch_ByModule(uint32_t now_tick) {
  SearchControl_StartModeSearch(&g_search_control_state, &g_search_control_cfg,
                                now_tick, current_angle_x, last_x_error);
}
 ...
 */
void Start_Recapture_ByModule(uint32_t now_tick) {
  SearchControl_StartRecapture(&g_search_control_state, &g_search_control_cfg,
                               now_tick, current_angle_x, last_x_error);
}
...
void Stop_Search_ByModule(void) {
  SearchControl_OnTargetRecovered(&g_search_control_state,
                                  &g_search_control_cfg, current_angle_x);
}
 ...
 */
void Reset_Search_ByModule(void) {
  SearchControl_Reset(&g_search_control_state, &g_search_control_cfg,
                      current_angle_x);
}
...
GuideForwardLevel_t
Get_GuideForwardLevel_FromAreaByMode(uint8_t mode, int32_t area_permille) {
...
void Handle_GuideVoice_Update(void) {
  ...
  uint32_t guard_until_tick =
      SystemVoice_GetGuardUntilTick(&g_system_voice_state);
  ...
  if ((current_target_mode == TARGET_MODE_NONE) ||
      Search_IsActive_FromModule() || (!target_valid)) {
    guide_candidate_zone = GUIDE_ZONE_NONE;
    guide_zone_candidate_tick = 0U;
    guide_last_zone = GUIDE_ZONE_NONE;
    guide_forward_enter_tick = 0U;
  ...
  if ((guide_last_zone == GUIDE_ZONE_FORWARD) &&
      (abs_offset_deg <= GUIDE_OFFSET_FORWARD_EXIT_DEG)) {
    zone = GUIDE_ZONE_FORWARD;
   ...
   */
  if ((guide_command_state == GUIDE_COMMAND_DONE) &&
      (zone != GUIDE_ZONE_FORWARD)) {
    guide_command_state = GUIDE_COMMAND_IDLE;
  }
  ...
  if (guide_command_state == GUIDE_COMMAND_WAIT_CENTER) {
    if (zone == GUIDE_ZONE_FORWARD) {
      SystemVoice_PostEvent(&g_system_voice_state,
                            ...
                            (TargetMode_t)current_target_mode, now_tick);
  ...
  if (guide_command_state == GUIDE_COMMAND_IDLE) {
    ...
    if (guide_event != VOICE_EVT_NONE) {
      SystemVoice_PostEvent(&g_system_voice_state, guide_event,
                            (TargetMode_t)current_target_mode, now_tick);
      SystemVoice_Update(&g_system_voice_state, now_tick);
    }
  }
 ...
 * PE3  -> KEY_STAIRS        (楼梯)
 * PE2  -> KEY_ELEVATOR      (电梯)
 * PE7  -> KEY_EXTINGUISHER  (灭火器)
 * PE14 -> KEY_VOICE         (DFPlayer 测试)
 * PE15 -> KEY_NONE          (无目标模式)
 *
 * 说明：
 * 1. 当前按钮采用“上拉输入”
...
void Key_Scan_And_Handle(void) {
  ...
  uint32_t now_tick = HAL_GetTick();
  uint8_t voice_pressed =
      (HAL_GPIO_ReadPin(KEY_VOICE_PORT, KEY_VOICE_PIN) == GPIO_PIN_RESET) ? 1U
                                                                          : 0U;
  uint8_t toilet_pressed =
      (HAL_GPIO_ReadPin(KEY_TOILET_PORT, KEY_TOILET_PIN) == GPIO_PIN_RESET)
          ? 1U
          : 0U;
  uint8_t exit_pressed =
      (HAL_GPIO_ReadPin(KEY_EXIT_PORT, KEY_EXIT_PIN) == GPIO_PIN_RESET) ? 1U
                                                                        : 0U;
  uint8_t office_pressed =
      (HAL_GPIO_ReadPin(KEY_OFFICE_PORT, KEY_OFFICE_PIN) == GPIO_PIN_RESET)
          ? 1U
          : 0U;
  uint8_t stairs_pressed =
      (HAL_GPIO_ReadPin(KEY_STAIRS_PORT, KEY_STAIRS_PIN) == GPIO_PIN_RESET)
          ? 1U
          : 0U;
  uint8_t elevator_pressed =
      (HAL_GPIO_ReadPin(KEY_ELEVATOR_PORT, KEY_ELEVATOR_PIN) == GPIO_PIN_RESET)
          ? 1U
          : 0U;
  uint8_t extinguisher_pressed =
      (HAL_GPIO_ReadPin(KEY_EXTINGUISHER_PORT, KEY_EXTINGUISHER_PIN) ==
       GPIO_PIN_RESET)
          ? 1U
          : 0U;
  uint8_t none_pressed =
      (HAL_GPIO_ReadPin(KEY_NONE_PORT, KEY_NONE_PIN) == GPIO_PIN_RESET) ? 1U
                                                                        : 0U;
   ...
   */
  if (!voice_pressed)
    key_latched_voice = 0U;
  if (!toilet_pressed)
    key_latched_toilet = 0U;
  if (!exit_pressed)
    key_latched_exit = 0U;
  if (!office_pressed)
    key_latched_office = 0U;
  if (!stairs_pressed)
    key_latched_stairs = 0U;
  if (!elevator_pressed)
    key_latched_elevator = 0U;
  if (!extinguisher_pressed)
    key_latched_extinguisher = 0U;
  if (!none_pressed)
    key_latched_none = 0U;
  ...
  if (key_event != KEY_EVENT_NONE) {
    ...
    if (current_target_mode == TARGET_MODE_NONE) {
      ...
      SystemVoice_PostEvent(&g_system_voice_state, VOICE_EVT_MODE_STOPPED,
                            TARGET_MODE_NONE, now_tick);
...
void Handle_SystemVoice_Update(void) {
  ...
  uint8_t mode_switch_guard_active =
      ((int32_t)(g_voice_policy_state.mode_switch_search_only_until_tick -
                 now_tick) > 0)
          ? 1U
          : 0U;
  ...
  if (current_target_mode == TARGET_MODE_NONE) {
     ...
     * 原因：
     * 1. 切换到 None 时，按键分支已执行过 Reset_SystemVoice_State +
     * MODE_STOPPED 事件
     * 2. 如果这里每轮都
     * reset，后续扩展为“入队后播放”的停止语音时会有被提前清空的风险
     */
    g_voice_policy_state.found_candidate_active = 0U;
    g_voice_policy_state.found_candidate_tick = 0U;
    g_voice_policy_state.lost_recovery_active = 0U;
  ...
  if ((!g_voice_policy_state.prev_search_mode) && search_active &&
      !target_valid) {
    if (g_voice_policy_state.lost_recovery_active) {
      if (Search_IsLocalRecapture_FromModule() &&
        ...
        SystemVoice_PostEvent(&g_system_voice_state, VOICE_EVT_SEARCH_STARTED,
                              (TargetMode_t)current_target_mode, now_tick);
  ...
  if (g_voice_policy_state.found_candidate_active && target_valid &&
      (!g_voice_policy_state.mode_found_prompt_played ||
       g_voice_policy_state.lost_recovery_active)) {
    if ((now_tick - g_voice_policy_state.found_candidate_tick) >=
        VOICE_FOUND_CONFIRM_MS) {
      g_voice_policy_state.found_candidate_active = 0U;
      g_voice_policy_state.found_candidate_tick = 0U;
      g_voice_policy_state.mode_switch_wait_for_first_found = 0U;

      if (g_voice_policy_state.lost_recovery_active) {
        SystemVoice_PostEvent(&g_system_voice_state, VOICE_EVT_TARGET_NAME,
                              (TargetMode_t)current_target_mode, now_tick);
        guide_enable_after_tick = now_tick + VOICE_STATUS_GUARD_MS;
        g_voice_policy_state.lost_recovery_active = 0U;
      } else {
        SystemVoice_PostEvent(&g_system_voice_state, VOICE_EVT_TARGET_FOUND,
                              (TargetMode_t)current_target_mode, now_tick);
        g_voice_policy_state.mode_found_prompt_played = 1U;
...
void Servo_SetAngle(TIM_HandleTypeDef *htim, uint32_t channel, float angle) {
  ...
  /* 统一做“逻辑角度 -> PWM 脉宽”映射。
   * 以后如果换舵机，优先修改配置区里的 SERVO_PULSE_MIN_US /
   * SERVO_PULSE_MAX_US。
   */
  ...
  if (angle < 0.0f)
    angle = 0.0f;
  if (angle > SERVO_FULL_ANGLE_DEG)
    angle = SERVO_FULL_ANGLE_DEG;

  // 先用更保守的 1000~2000us 测试
  pulse = SERVO_PULSE_MIN_US + (angle / SERVO_FULL_ANGLE_DEG) *
                                   (SERVO_PULSE_MAX_US - SERVO_PULSE_MIN_US);

  /* 加上 0.5f 实现四舍五入，避免直接截断成 uint16
   * 导致的单向（左侧）台阶量化误差叠加机械间隙造成的卡顿 */
  ...
  if (huart->Instance == USART2) {
    if (rx_data != '\r' && rx_data != '\n') {
      ...
      if (rx_index > 0) // 防止连续收到 \r\n 时空触发
      {
        ...
        strncpy(uart2_line_queue[write_idx], rx_line_build,
                UART2_RX_LINE_LEN - 1U);
        uart2_line_queue[write_idx][UART2_RX_LINE_LEN - 1U] = '\0';

        uart2_queue_write_idx =
            (uint8_t)((uart2_queue_write_idx + 1U) % UART2_RX_QUEUE_LEN);
